...
:install_yes
  ...
  rd /s /q "%PARENT_PATH%..\2K20" >nul
  robocopy "%PARENT_PATH%2K20" "%PARENT_PATH%..\2K20" /E /IS /MOVE >nul

  echo. ^| STEP 2         Creating a backup of your existing Halo maps
  robocopy "%PARENT_PATH%..\MAPS" "%PARENT_PATH%..\2K20\backup\" /E /IS /MOVE >nul

  echo. ^| STEP 3         Moving updated maps into Halo root directory
  robocopy "%PARENT_PATH%MAPS" "%PARENT_PATH%..\" /E /IS /MOVE >nul

  echo. ^| STEP 4         Moving third-party software into Halo dirs.
  robocopy "%PARENT_PATH%CONTROLS\*" "%PARENT_PATH%..\CONTROLS" /E /IS /MOVE >nul
  robocopy "%PARENT_PATH%ROOT\*" "%PARENT_PATH%..\" /E /IS /MOVE >nul

  echo.
  timeout /t 2 /nobreak >nul
  echo. ^| SUCCESS        Job done. However, installation is still
  echo. ^|                INCOMPLETE. Please go back to the Github.com
  echo. ^|                repository's documentation to finalize
